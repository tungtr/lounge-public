...
const Verify = ({
  ...
  const verifyAccount = async () => {
    try {
      const apiResponse = await axios.post(`${process.env.NEXT_PUBLIC_CLIENT_HOST}/api/auth/verify`, { token });
  ...
  const resendVerificationEmail = async () => {
    ...
    try {
      const apiResponse = await axios.post(`${process.env.NEXT_PUBLIC_CLIENT_HOST}/api/auth/verify/send`, { email: user.email });
      if (apiResponse.data.status === HttpStatusCodes.OK) {
        showNotification({
          title,
